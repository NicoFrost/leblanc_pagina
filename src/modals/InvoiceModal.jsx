...
export const InvoiceModal = () => {
    ...
    const {isInvoiceModalOpen,closeInvoiceModal} = useUIStore()
    const {activeInvoice,setActiveInvoice,invoices} = useInvoiceStore()
        ...
        >
            <Box>
                <ModalLayout offset='-100%' width={500}>
                    ...
                    <Box>
                        <Typography>{activeInvoice.amount}</Typography>
                        <List>
                            <ListSubheader sx={{color:"black",fontWeight:800,fontSize:"20px"}}>Cobranzas</ListSubheader>
                            {
                            collections?.map((c) => {
                                if(c.invoiceId == activeInvoice.id) {
                                    ...
                                    return (
                                        <ListItemButton sx={{':hover':{backgroundColor: getMethodById(c.methodId)?.color || '#dddada'},padding: "5px",backgroundColor: getMethodById(c.methodId)?.color ? `${getMethodById(c.methodId).color}b3` : 'white' }} key={c.id} selected={selectedIndex.id === c.id} onClick={() => setSelectedIndex(c)}>
                                            ...
                                            <ListItemText secondary={"Importe: " + c.amount} primary={`Fecha: ${c.date.format('dddd DD/MM')}`} />
                                        </ListItemButton>
                                    )
                                }
                            })
